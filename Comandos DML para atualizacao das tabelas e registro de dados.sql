...
--Adição do valor total da compra na tabela carrinho

ALTER TABLE carrinho ADD valor_total REAL

SELECT c.cod_carrinho, c.quant_item, p.valor_unitario, (c.quant_item * p.valor_unitario) AS total 
FROM carrinho c, produto p 
WHERE p.cod_produto = c.cod_produto 

UPDATE carrinho 
SET valor_total = (
SELECT (carrinho.quant_item * produto.valor_unitario) 
FROM carrinho, produto 
WHERE produto.cod_produto = carrinho.cod_produto)
WHERE cod_carrinho = 1
